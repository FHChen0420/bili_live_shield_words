...
words =  [ 
    ### 一般屏蔽词（易变动）
    "即位", "在任", "下台", "倒台", "候选", "选举", 
    "总理", "总统", "领袖", "领导", "纪委", "政府", "国会", "议会",
    "修正", "特权", "政策", "提案", "红通", "腐败", "出访", "审查", 
    "天安", "安门", "赤字", "民众", "中共", "国歌", "金砖", "大会", "会议", 
    ...
    "boki", "dang", "drug", "fuck", "knee", "kuma", "loli", "nmsl", "rori", "sina", "tank", "yuan",
    ...
    "6年", "31年", "80年代", "110吗", "1月23", "7月1日", "7月5日", "7月13", "8月17日", "12月28", "命运共同体", "克里斯托弗", "你是你我是我", #汉字格式的日期也会被屏蔽，这里没写出来
    ...
    "手#5指#5插", "徐#5上#5爽", "许#5艾#5莉", "谢#5日#5双", "[鲁撸露]#5一#5发", "[徐许]#5[上玩]#5[碧双霜]",
    "[日草艹操干曰死烧解透跳]#6[你尼我他她它]#5[妈马吗码蚂玛犸嘛母m家]", "文#6古#6花", "看#6地#6方", "不#6钱#6[啊3]", "[.]#6c#6c", "[谢x]#6s#6w",
    ...
    "吉#1[尔儿]", "野#1[爹妈]", "射#1[爆爽]", "断#1[手脖]", "暴#1[饮食]", "称#1[王皇帝]", "微#1[博搏勃]", "奶#1[头汁水]", "涩#1[气批片p]", "精#1[美日湛子]", 
    ...
    "网#2恋", "巨#2乳", "自#2尽", "涩#2情", "逼#2真", "翻#2墙", "蓝#2灯", "人#2[权妖]", "[逼b]#2[黑毛]", "黑#2[逼b]", "[傻沙煞撒]#2[逼比笔]",
    "支#3那", "去#3搜", "百#3d", "自#3杀", "共#3产", "毛#3东", "手#3银", "涩#3图", "肉#3棒", "伪#3娘", "邪#3教", "果#3聊", "裸#3体", "粉#3奶", "内#3射", "脸#3丑", "子#3宫",
    "鸡#3[巴八8⑧]", "[涩色]#3网", "被#3[日草艹操曰]", "[习習吊弔]#3大", "[草艹操]#3b", "[下压]#3注", "[黄色h]#3片", "[做作坐座]#3爱", 
    ...
    "留#5水", "性#5爱", "威#5雄",
    "罗#6莉", "宽#6衣", "快#6死", "彭#6s", "[彭澎p]#6帅", "萝#6[利俐]", "[逼b]#6紧", "[习習]#6[近进]",
    "幼#8[比逼b]",
    ...
    ### 以下屏蔽词已做其它处理（见rules）
    # "hk", "ma", "tw", "xi", "zf", "abs", "sex", "tam", "xjp", "xyz", "anal", "arms", "asmr", "fldf", "ntop", "baidu", "antifa", "tmmsme", "yayeae",
...
rules = {
    ...
    "(?i)(m ?)(a)": lambda x: x.group(1) + letter[x.group(2)],
    ...
    "[嘴脸鼻眼脑舌](?=[^ 0-9]{0,3}难[听看])": add_space,
    "[你您他她这那个](?=[^ 0-9]{0,3}([胖矮丑]|难[听看]))": add_space,
    "[你您他她这那](?=[^ 0-9]{0,2}[嘴脸鼻眼脑舌][^ 0-9]{0,2}烦)": add_space,
    "牲 ?畜": "牲 1畜",
    "(?<![大是])猩猩%s"%(f2): "猩 猩",
    "%s猴子%s"%(f1,f3): "猴 子",
    "%s死%s"%(f1,"(?=[^ 2-79\u4E00-\u9FA5]*$)"): "Si",
    ...
    "[日草艹操干曰死烧解透跳](?=.*?[你尼我他她它].*?[妈马吗码蚂玛犸嘛母m家])": add_space,
    "好多(?=[^ 0-9]?猴)": add_space,
    "眼(?=[^ 0-9]?睁)": add_space,
    "恶(?=[^ ]?[心丑])": add_space,
    "滚(?=[^ ]?[滚开])": add_space,
    "丑(?=[^ ]?[恶死女男吧吗嘛啊呢哦么])": add_space,
    "死(?=[^ ]?[摔心狗吧吗嘛啊呢哦么宅])": add_space,
    "摔(?=[^ ]?死)": add_space,
