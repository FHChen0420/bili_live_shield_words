...
words =  [ 
    ...
    "hw", "gc", "cjp", "cnm", "ghs", "kui", "lsp", "nmb", "nmd", "ply", "tmd", "usl", "xxd",
    "boki", "dang", "drug", "fuck", "isis", "knee", "kuma", "loli", "sina", "yuan",
    "bajiu", "bitch", "luoli", "obama", "sager", "secom", "shina", "hentai", "reddit", "signal", "tiktok",
    ...
    "波涛汹涌", "我是黄金", "尸体派对", "没有敌人", "少女之心", "奥斯曼人", "奥数魔刃", "奥数魔人", "孩子的鞋", "花花公子",
    "不想回忆", "最大限度",
    "1月23", "7月1日", "7月5日", "7月13", "8月17日", "12月28", "你是你我是我", #汉字格式的日期也会被屏蔽，这里没写出来

    ### 字符间隔相关
    "回#1来#3谢", "姐#1姐#5[逼b]", "妹#1妹#5[逼b]", "[逼b]#3看#1吗", "搜#3这#1个", "投#3比#1赛", "看#3头#1像", "同#3性#3恋",
    "道#3上#3飞", "名#3字#3看", "我#3是#3处", "下#3面#3好", "[白干]#3一#3下", "少#3女#3[下自]", "直#3播#3[日草操曰]",
    "射#4身#1上", "童#4收#1养", "买#4烟#1花", "删#4评#1论", "v#4b#3o", "天#4安#4门", "萝#4莉#4控", "手#5指#5插", "[鲁撸露]#5一#5发", "徐#5上#5爽",
    "[日草操干曰死烧解]#6[你我他她它]#5[妈吗马嘛母m]", "r#1i#6[你我他她它]#5[妈吗马嘛母m]", "文#6古#6花", "看#6地#6方", "不#6钱#6[啊3]",
    "准#1备#3纸#1巾", "那#1个#4奶#1奶", "羊#1羊#4结#1婚", "学#1生#4学#1生", "妈#1妈#6唱#1歌", "不#1论#6生#1死", "找#1工#1作#3加",
    "清#6透#6世#6界", "w#4e#4i#4b#4o", "一#6个#6人#6寂#6寞",
    
    "双#1[奶乳]", "玩#1[逼奶]", "吉#1[尔儿]", "射#1[爆爽]", "法#1[会能弄]", "乳#1[头首量]", "称#1[王皇帝]", "涩#1[气批片p]",
    "大#1[吊弔麻胸]", "点#1[人1cfl]", "死#1[吧妈法ね]", "色#1[图情皮批逼戒b]",
    "[逼b]#1里", "[批阴]#1毛", "[左右]#1倾", "[两二2]#1会", "[杀去爽]#1死", "[处下熟]#1女", "[贫小平]#1乳", "[调传宗]#1教", "[鸡己性]#1吧", "[湿射硬]#1了", "[条包孢窑梯]#1子",
    "[插吸]#1[你他她它]", "[.。·]#1[cf]", "[傻沙煞撒]#1[逼比笔]",
    "网#2恋", "巨#2乳", "自#2尽", "涩#2情", "人#2权", "[逼b]#2毛", "湿#2视#2频",
    "支#3那", "去#3搜", "百#3d", "自#3杀", "共#3产", "毛#3东", "手#3银", "涩#3图", "肉#3棒",
    "[涩色]#3网", "被#3[日草操曰]", "[习習吊弔]#3大", "[草操]#3b", "[下压]#3注", "[黄色h]#3片", "[做作坐座]#3爱", 
    "六#4四", "八#4九", "车#4震", "援#4交", "后#4入", "天#4门", "流#4世", "主#4席", "黄#4网", "明#4[泽z]", "[小进]#4平", "[连再]#4任", "[看好]#4胸", "[日草操干曰]#4[姐妹奶]", 
    "留#5水", "性#5爱", "威#5雄",
...
rules = {
    ...
    " +" :" ",
    ### 单字/特殊字符
    "翠": "翆", "尻": "𡱧", "爬": "Pa", "痒": "𤶪", "淫":"Yin", "岿": "巍", "屌": "吊", "蛤": "Ha", "党": "Dαng", "奠": "Dian",
    ...
    "([干日草操曰].*?)(视.*?)(频)": lambda x: x.group(1)+fill(x.group(2),2)+x.group(3), # "[干日草操曰]#7视#1频"
    "([日草曰操].*?)(公.*?)(主)": lambda x: x.group(1)+fill(x.group(2),2)+x.group(3), # "操#3公#1主" "[日草曰]#9公#1主"
    ...
    "([徐许].*?)([上玩].*?)([碧双霜])": lambda x: # “徐#5上#5爽”不适用于此条规则
        (fill(x.group(1),6+r_pos(x.group(1),"徐许"))+x.group(2)+x.group(3))
        if measure(x.group(1),6+r_pos(x.group(1),"徐许")) and measure(x.group(2),6) else x.group(),
    ### 保护型处理规则
