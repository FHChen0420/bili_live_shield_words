...
words =  [ 
    ...
    "吉#1[尔儿]", "野#1[爹妈]", "射#1[爆爽]", "暴#1[饮食]", "称#1[王皇帝]", "微#1[博搏勃]", "奶#1[头汁水]", "涩#1[气批片p]", "精#1[美日湛子]", 
    ...
    "支#3那", "去#3搜", "百#3d", "自#3杀", "共#3产", "毛#3东", "手#3银", "涩#3图", "肉#3棒", "伪#3娘", "邪#3教", "果#3聊", "裸#3体", "粉#3奶", "内#3射", "子#3宫",
    "鸡#3[巴八8⑧]", "[涩色]#3网", "被#3[日草艹操曰]", "[习習吊弔]#3大", "[草艹操]#3b", "[下压]#3注", "[黄色h]#3片", "[做作坐座]#3爱", 
    "六#4四", "八#4九", "车#4震", "援#4交", "后#4入", "流#4世", "主#4席", "黄#4网", "阴#4道", "赤#4毒", "近#4评", "孤#4儿", "倒#4车",
    "明#4[泽z]", "近#4苹", "[小进]#4平", "[连再]#4任", "[看好]#4胸", "吃#4[比逼币笔]", "[日草艹操干曰吃]#4[姐妹奶姨吊弔]", 
    "留#5水", "性#5爱", "威#5雄",
    "罗#6莉", "宽#6衣", "彭#6s", "[彭澎p]#6帅", "萝#6[利俐]", "[逼b]#6紧", "[习習]#6[近进]",
    "幼#8[比逼b]",
    ...
    # "6年", "71", "1921", "av", "g7", "ma", "sb", "tg", "小学生", "不习惯", "发不出", "就去泡", "莉莉安", "风平浪静", "老不死的",
]
...
rules = {
    ...
    "[嘴脸鼻眼脑舌](?=[^ 0-9]{0,3}(难[听看]|[烦丑]))": add_space,
    "[你您他她这那个](?=[^ 0-9]{0,3}([胖矮丑烦]|难[听看]))": add_space,
    "(?<![大是])猩猩%s"%(f2): "猩 猩",
    "%s猴子%s"%(f1,f3): "猴 子",
    "%s死%s"%(f1,"(?=[^ 2-79\u4E00-\u9FA5]*$)"): "Si",
    "一狗": "一 狗",
