...
words =  [ 
    ...
    "健身教练", "为所欲为", "特别待遇", "人生经验", "银河联邦", "活塞运动", "再来一次", "夫目前犯", "自产自销",
    "黑暗之魂", "求生之路", "上古卷轴", "侠盗飞车", "尸体派对", "动物之森", "生化危机",
    ...
    "初#1[心生]", "吉#1[尔儿]", "好#1[爸爹]", "野#1[爹妈]", "射#1[爆爽]", "黄#1[图游]", "虐#1[待杀]", "暴#1[饮食动乱徒民]", "就#1[职職]", "腐#1[败敗]", "回#1[逼b]", "称#1[王皇帝神]", "微#1[博搏勃]", "奶#1[头汁水]", "想#1[日草艹曰射]",
    "涩#1[气批片p]", "[你送死]#1妈", "精#1[美日湛子]", "乳#1[头首量水摇沟]", "魂#1[一二三123]", "大#1[麻吊弔胸波奶胃法选]", "点#1[人1cfl]", "死#1[吧法ね]", "色#1[图情皮批逼戒狼b]",
    "[逼b]#1里", "[批阴]#1毛", "[吞吃]#1精", "[左右]#1倾", "[狂猛]#1吃", "[红紅]#1[魔通]", "[施受]#1虐",
...
rules = {
    ...
    "%s死(?![\s%s])"%(f1,cn): "si",
    "恶(?=\S?[心啊吧吗嘛么])": "悪",
    "([真太很好]|有?一?点)(?=[^\s不]{0,3}[胖矮傻笨蠢病疯丑矮菜怪废脏])": lambda x: " "+x.group(),
    
}

list_1=[
    "母韵", "全家", "彩笔", "柴犬", "狒狒", "闭嘴", "双亲", "渣女", "股间",
    "矮子", "脑子", "大妈", "好吵", "长得", "肥仔", "崽子", "整容", "死宅",
    "脏话", "男妖", "老太婆", "脏东西", "大舌头",
    "难[看听]", "滚[滚开]", "睁[开眼]", "残[疾障]", "垃圾(?!游戏)",
]
...
list_3=[
    "(?<!\s)狗(?!狗)", "(?<!\s)雑", "(?<![\s繁])琐", "(?<!愚)蠢(?![蠢货])", "(?<![小文])丑", "个样子?\s?",
