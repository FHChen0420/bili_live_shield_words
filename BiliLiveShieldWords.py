...
pm=".,!?+*/#%&^$`(){}…@~\"\'\\;:<>|=·。，！？；：￥“”‘’—（）【】「」\-\[\]" # 常见标点符号，用于正则表达式的[]内
jp="\u0800-\u4E00" # 日文，用于正则表达式的[]内
cn="\u4E00-\u9FA5" # 中文，用于正则表达式的[]内
...
words =  [ 
    ...
    "膝下", "扶她", "看片", "凸起",
    "肥猪", "下贱", "月半", "嘴臭", "拉屎", "撤硕", "低俗", "憨批", "作死",
    "刁大", "禽兽", "畜生", "吃屎", "智障", "小丑", "杂种", "猥琐", "废物", "愚民",
    ...
    "gc", "hw", "hk", "qd", "rh", "zf",
    "abs", "cjp", "cnm", "cov", "gay", "ghs", "hso", "kui", "lsp", "nmb", "nmd", "ntr", "ply", "roc", "scp", "soe", "tmd", "usl", "wic", "wjb", "xxd",
    ...
    "31年", "80年代", "110吗", "1月23", "7月1日", "7月5日", "7月13", "8月17日", "12月28", "十月十",
    ...
    ### 字符间隔相关
    "奥#1数#1[魔默]", "一#1口#1[气吃喝]", "收#1[妹弟女]#1[妹弟儿]", "[有搞]#1黄#1色", "[一二三四五六]#1代#1目",
    ...
    "[你尼]#4[妈马吗码蚂玛犸嘛母m家]#4[死币比逼必猪狗b]", "[大小妈姐妹哥弟一二三四五六七八九]#4[小姐妹哥弟一二三四五六七八九]#5[在来做进]",
    ...
    "吉#1[尔儿]", "野#1[爹妈]", "射#1[爆爽]", "暴#1[饮食动乱徒民]", "就#1[职職]", "腐#1[败敗]", "称#1[王皇帝]", "微#1[博搏勃]", "奶#1[头汁水]", "想#1[日草艹曰射]",
    "涩#1[气批片p]", "[你送死干]#1妈", "精#1[美日湛子]", "乳#1[头首量水摇沟]", "魂#1[一二三123]", "大#1[麻吊弔胸波奶胃]", "点#1[人1cfl]", "死#1[吧法ね]", "色#1[图情皮批逼戒狼b]",
    ...
    "留#5水", "性#5爱", "威#5雄",
    "罗#6莉", "宽#6衣", "刀#6卖", "彭#6s", "[彭澎p]#6帅", "萝#6[利俐]", "[逼b]#6紧", "[习習]#6[近进]", "幼#8[比逼b]",
...
rules = {
    ...
    ### 中文非常规处理规则
    "([草艹操日][ %s]*)([你我他她它比笔逼]|时光|女儿)"%pm: lambda x: x.group(1)+sp+x.group(2),
    ...
    "金(?=[ %s]*[三四])"%pm: lambda x: x.group()+sp,
    "工(?=[ %s]*期)"%pm: lambda x: x.group()+sp,
    "公(?=[ %s]*安)"%pm: lambda x: x.group()+sp,
    "巨(?=[ %sa-zA-Z]*婴)"%pm: lambda x: x.group()+"1",
    ### 保护型处理规则
    ...
    "尼嚎": "你好", "人妖": "人Yao", "咖喱 ?人": "咖喱Ren", "全 ?家 ?炸": "全 家Zha", "神经质": "神经 质",
    "牲畜": "牲%s 畜"%sp, "快死": "快%s 死"%sp, "坦克": "坦%s 克"%sp, "(?<![小文])丑":"chou", "(?<!愚)蠢(?!蠢?欲动)":"chun",
    "(?<=[笨傻菜])[逼Bb]": "Ⲃ", "变态":"変态", "臭小鬼": "臭小鬼",
    "%s死(?![\s2-79\u4E00-\u9FA5])"%(f1): "Si",
    "(?i)(g[%s]) ?(c)"%(pm): lambda x: x.group(1)+" "+sp+x.group(2),
    "(恶[^\s心]?)心": lambda x: " "+x.group(1)+"xin",
    
}

insert_space_list1=[
    "母韵", "太笨", "全家", "彩笔", "柴犬", "狒狒", "闭嘴", "双亲", "渣女", "股间", 
...
add_space_list=[
    "(真|有点)(?=[^\s不]{0,3}[傻笨蠢病疯丑矮菜怪])",
    "[嘴脸鼻眼脑舌肚](?=[^\s\d]{0,6}?[胖矮丑烦笨傻蠢怪臭土大睁垃混])",
    "[你您他她这那个][们么样种些]?(?=[^\s\d个]{0,3}?[脸嘴鼻脑舌肚货猴胖矮丑烦笨傻蠢怪臭土歪睁垃混废Pp])",
    "[你您他这那]们?(?=.*?位.*?要求)",
    "[你您他她]们?(?=.*?东西)",
    "睁(?=[^\s睁]{0,3}?[开嘴脸鼻眼脑舌])",
    "[脸脑鼻口](?=.*?像)",
    ...
    "你们?(?=[^\s\d吓笑]{0,6}死)",
    "死(?=\S?[心狗吧吗嘛啊呢哦么宅])",
    "[病笨傻蠢臭](?=[^\s\d]{0,2}[样人吧吗嘛啊呢哦么了])",
    "[胖矮](?=\S{0,3}?样)",
    "[笨傻](?=.{0,2}[逼比笔币Bb])",
    "%s菜(?=[%sa-zA-Z]*[比笔币逼])"%(f1,pm),
    "[猪狗](?=\S{0,2}种)",
    "[一条](?=[^\s\d条]{0,2}狗)",
    "雑(?! )",
    "\d(?=.*?字母表)",
