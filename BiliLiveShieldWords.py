...
words =  [ 
    ...
    "十字弓", "走后门", "敏感词", "委员会", "委員会", "检察院", "起风了", "都在笑", "乡下人",
    ...
    ### 字符间隔相关
    "奥#1数#1[魔默]", "一#1口#1[气吃喝]", "收#1[妹弟女]#1[妹弟儿]", "搞#1黄#1色", "[色涩]#1咪#1咪", "[一二三四五六]#1代#1目",
    "下#1面#2[松紧]", "回#1来#3谢", "观#1众#3v", "还#1没#3封", "[姐妹]#1[姐妹]#5[逼b]", "弟#1弟#6大", "护#1照#6办", "飞#1机#6好",
    "射#2出#1来", "湿#2视#2频", "[01]#2找#2[01]",
    "[逼b]#3看#1吗", "搜#3这#1个", "投#3比#1赛", "看#3[头昵内i]#1[像称裤d]", "戴#3绿#1帽", "吃#3菊#1花", "摸#3屁#1股", "h#3动#1[画漫]", "6#3月#24",
    ...
    "[01]#2还#1是#2[01]", "权#2利#2斗#2争", "你#3画#3我#3猜", "闭#3关#3锁#3国", "不#3穿#3衣#3服", "看#3我#3i#1d", "看#6我#6[头昵]#6[昵像]", "清#6透#6世#6界", "想#6p#1a#1p#1a", "一#6个#6人#6寂#6寞", "不#1要#6这#1种#1事#1情", "1#10#3月#31#10",
    ...
    "初#1[心生]", "吉#1[尔儿]", "好#1[爸爹]", "野#1[爹妈]", "射#1[爆爽]", "黄#1[图游]", "暴#1[饮食动乱徒民]", "就#1[职職]", "腐#1[败敗]", "回#1[逼b]", "称#1[王皇帝神]", "微#1[博搏勃]", "奶#1[头汁水]", "想#1[日草艹曰射]",
    "涩#1[气批片p]", "[你送死]#1妈", "精#1[美日湛子]", "乳#1[头首量水摇沟]", "魂#1[一二三123]", "大#1[麻吊弔胸波奶胃法选]", "点#1[人1cfl]", "死#1[吧法ね]", "色#1[图情皮批逼戒狼b]",
    "[逼b]#1里", "[批阴]#1毛", "[吞吃]#1精", "[左右]#1倾", "[狂猛]#1吃", "[红紅]#1[魔通]", 
...
rules = {
    ...
    "%s死(?![\s2-79%s])"%(f1,cn): "si",
    "恶(?=\S?[心啊吧吗嘛么])": "悪",
    "([真太很好]|有点)(?=[^\s不]{0,3}[胖矮傻笨蠢病疯丑矮菜怪废脏])": lambda x: " "+x.group(),
...
add_space_list=[
    ...
    "[妈马吗码蚂玛犸嘛](?=[^\s\d妈吗嘛]?没)",
    "(有[^\s\d]{0,3})(?=(?<!中二)病)",
    "[%s你您](?=[^\s\d]?[%s])"%(hz_ni,hz_ma),
    "[%s](?=[^\s\d]?[%s您])"%(hz_ma,hz_ni),
    "你们?(?=[^\s\d吓笑]{0,6}死)",
    "死(?=\S?[心狗猪吧吗嘛啊呢哦呀么宅])",
    "[病笨傻蠢臭胖矮菜狗猪猴]蛋?(?=\S{0,3}[样人吧吗嘛啊呢哦呀么了])",
    "[菜笨傻胖](?=.{0,2}[狗猪逼比笔币Bb])",
    "\d(?=.*?字母表)",
    "残(?=[疾障%s])"%(jp),
...
ex_rules={
    ...
    "病": "Ⲃing", "蠢": "chun", "丑": "chou", "死": f"s{sp}i", "炸": "zha", "呕": "ou",
