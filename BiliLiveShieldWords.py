...
hz_liu_lu="六刘柳浏流留琉硫碌溜馏遛榴瘤6⑥卢庐芦陆卤虏炉录鸬赂鹿颅绿鲁禄鲈路噜撸辘戮橹璐鹭露" # 重复：六碌
hz_si_shi="四司丝死寺似私祀伺饲驷食思斯肆嗣厮撕嘶4④十尸士氏什示矢石史市失仕世式师时识始饰视鸤虱实驶事势使侍诗试施恃柿是蚀拭适室狮拾屎峙舐轼逝硕匙释湿谥弑誓嗜噬螫" # 重复：似食
...
words =  [ 
    ...
    "郭嘉", "果加", "菓加", "天安", "安门",
    "杀死", "囚禁", "施虐", "虐待", "捆绑", "割腕", "匕首", "疫情", "新冠", "大麻",
    "色戒", "湿了", "射了", "硬了", "中出", "高潮", "调教", "被透", "走光", "诱惑",
    "喘气", "喘息", "呻吟", "处女", "处男", "绅士", "抖m" , "h漫" , "性癖", "无码",
    "胖次", "欧派", "罩杯", "大胸", "嘿咻", "下女", "内内", 
    "肥猪", "下贱", "死吧", "你妈", "死妈", "去死", "月半", "己吧", "送妈",
    "台湾", "香港", "日吹", "精美", "精日", "弯弯", "油管", "推特", "微博", "动森",
    "鸣人",
    ...
    "不想活", "自由门", "咖啡因", "死灵魂", "白衬衫", "生理期", "空气炮", "黑历史", "就去泡", "一本道", "被传染",
    "纪念日", "为自由", "莉莉安", "李医生", "右大人", "绞肉机", "不唱歌", "女菩萨", "发不出", "老鼠台", "缘之空",
    "自由之门", "继续前进", "并肩同行", "焕然一新", "奥斯曼人", "二氧化碳", 
    "身经百战", "黑框眼镜", "谈笑风生", "活不下去", "飘飘欲仙", "动物之森", "分割人生",
    "你是你我是我",
    ...
    ### 字符间隔相关
    "v#4b#3o", "道#3上#3飞", "名#3字#3看", "我#3是#3处", "射#4身#1上", "天#4安#4门", 
    "文#6古#6花", "看#6地#6方", "清#6透#6世#6界", "一#6个#6人#6寂#6寞",
    "[贫小平]#1乳", "色#1[逼b]", "[傻沙煞撒]#1[逼比笔]", "插#1[你他她它]", "称#1[王皇帝]", "[.。·]#1[cf]", "点#1[1cfl]", 
    "巨#2乳", "[草操]#3b", "被#3[草曹操]", "少女#3[下自]", "[做作坐座]#3爱", "去#3搜", "下#3注", "百#3d",
    "[看好]#4胸", "车#4震", "小#4平", "再#4任", "明#4泽", "援#4交", "后#4入", "天#4门", "流#4世", "留#5水", "性#5爱",
    
    ### 拼音组合相关
    "[%s]#1都"%(hz_bai),
    "[%scx]#2[%s]"%(hz_xi,hz_pin_ping),
    "[%s]#2[%sp]"%(hz_xi,hz_pin_ping), # 分成两条是为了避免cp、xp被加点处理，导致字数消耗增加
    "[%s]#3[%s]"%(hz_ba,hz_jiu),
    "[%scx]#3[%s呼]"%(hz_xi,hz_jin_jing),
    "[%s]#3[%s呼乎p]"%(hz_jin_jing,hz_pin_ping), 
    "[%s]#3[%s舍捨]"%(hz_liu_lu,hz_si_shi),
]
...
rules = {
    ...
    "(?i)(六|6|⑥|l ?i ?u)(.*?)(四|肆|4|④|s ?i)": lambda x: x.group(1)+fill(x.group(2),4)+x.group(3), 
    "(?i)([大一].*?)([一哥].*?)([在来])": lambda x:
