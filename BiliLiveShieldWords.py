...
fill = lambda s,n: s+sp*(n-len(s)+s.count(" "))
...
rules = {
    ...
    "母韵": insert_space, "太笨": insert_space, "全家": insert_space, "彩笔": insert_space, "柴犬": insert_space,
    ...
    "好吵": insert_space, "长得": insert_space, "肥仔": insert_space, "崽子": insert_space,
    ...
    "%s爹妈%s"%(f1,f2): insert_space, "%s废柴%s"%(f1,f2): insert_space, "%s肥宅%s"%(f1,f2): insert_space, "%s孤勇者%s"%(f1,f2): insert_space,
    "%s吃的吗%s"%(f1,f2): insert_space, "%s断[手脖]%s"%(f1,f2): insert_space, "%s作[文者啊吧吗么]%s"%(f1,f2): insert_space,
    
    "真(?=[^\s不]{0,3}[傻笨蠢病疯丑矮菜])": add_space,
    "[嘴脸鼻眼脑舌肚](?=[^\s\d]{0,6}?[胖矮丑烦笨傻蠢怪臭土大睁垃混])": add_space,
    "[你您他她这那个][们么样种些]?(?=[^\s\d个]{0,3}?[脸嘴鼻脑舌肚货猴胖矮丑烦笨傻蠢怪臭土歪睁垃混废])": add_space,
    ...
    "[病笨傻蠢臭](?=[^\s\d]{0,2}[样人吧吗嘛啊呢哦么了])": add_space,
...
# <DATA END>
# 待处理:有点怪呢这个寿司  (备注:保持其他七个字完全不变,只换掉"怪"字,似乎可以查词性)
