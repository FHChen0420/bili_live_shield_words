...
words =  [ 
    ...
    "脑子瓦特", "恐怖漫画", "乡关何处", "有容乃大", "最后一课", "狼吞虎咽", "时间机器", "疲劳驾驶", "区别对待",
    "的混合物", "波涛汹涌", "报复社会", "官方签约", "我还活着", "贤者模式", "侃侃而谈", "约德尔人", "社会主义", "要开会了",
    "健身教练", "为所欲为", "特别待遇", "人生经验", "银河联邦", "活塞运动", "再来一次", "夫目前犯",
    "黑暗之魂", "求生之路", "上古卷轴", "侠盗飞车", "尸体派对", "动物之森",
    "31年", "80年代", "110吗", "1月23", "7月1日", "7月5日", "7月13", "8月17日", "12月28", "十月十", #汉字格式的日期也会被屏蔽，这里没写出来
    "命运共同体", "这就是中国", "克里斯托弗", "东京食尸鬼", "你是你我是我", "南无阿弥陀佛", "热爱105度",

    ### 字符间隔相关
    "奥#1数#1[魔默]", "一#1口#1[气吃喝]", "收#1[妹弟女]#1[妹弟儿]", "搞#1黄#1色", "[色涩]#1咪#1咪", "[一二三四五六]#1代#1目",
    "下#1面#2[松紧]", "回#1来#3谢", "观#1众#3v", "还#1没#3封", "[姐妹]#1[姐妹]#5[逼b]", "弟#1弟#6大", "护#1照#6办",
    "射#2出#1来", "湿#2视#2频", "[01]#2找#2[01]",
    "[逼b]#3看#1吗", "搜#3这#1个", "投#3比#1赛", "看#3头#1像", "看#3昵#1称", "看#3内#1裤", "戴#3绿#1帽", "吃#3菊#1花", "摸#3屁#1股", "h#3动#1[画漫]", "6#3月#24",
    "同#3性#3恋", "道#3上#3飞", "名#3字#3看", "我#3是#3处", "下#3面#3好", "习#3d#3d", "看#3洗#3澡", "爱#3萝#1莉", "少#3女#3[下自]", "直#3播#3[日草艹操曰]",
    "射#4身#1上", "童#4收#1养", "买#4烟#1花", "[删关]#4评#1论", "改#4中#1国", "天#4[%s]#4[门们闷]"%hz_an_1, "萝#4莉#4控", "正#4太#4控", "加#4速#4器", "习#4大#4大", 
    ...
    "找#1工#1作#3加", "准#1备#3纸#1巾", "都#1是#3衣#1服", "洗#1澡#3一#1起", "那#1个#4奶#1奶", "羊#1羊#4结#1婚", "学#1生#4学#1生", "妈#1妈#6唱#1歌", "不#1论#6生#1死", "[姐妹哥]#1[姐妹哥]#6大#1腿",
    "[01]#2还#1是#2[01]", "你#3画#3我#3猜", "闭#3关#3锁#3国", "不#3穿#3衣#3服", "清#6透#6世#6界", "一#6个#6人#6寂#6寞", "不#1要#6这#1种#1事#1情", "1#10#3月#31#10",
    ...
    "初#1[心生]", "吉#1[尔儿]", "野#1[爹妈]", "射#1[爆爽]", "黄#1[图游]", "暴#1[饮食动乱徒民]", "就#1[职職]", "腐#1[败敗]", "称#1[王皇帝神]", "微#1[博搏勃]", "奶#1[头汁水]", "想#1[日草艹曰射]",
    "涩#1[气批片p]", "[你送死干]#1妈", "精#1[美日湛子]", "乳#1[头首量水摇沟]", "魂#1[一二三123]", "大#1[麻吊弔胸波奶胃法]", "点#1[人1cfl]", "死#1[吧法ね]", "色#1[图情皮批逼戒狼b]",
    ...
    "[兄弟妹]#1控", "[贫小平双]#1乳", "[条包孢窑梯秃矮]#1子", "[杀去爽干操草爹妈饿想该]#1死", "[插吸]#1[你他她它]", "[.。·]#1[cf]",
    "网#2恋", "巨#2乳", "自#2尽", "涩#2情", "翻#2墙", "蓝#2灯", "渣#2男", "人#2权", "[逼b]#2[真黑毛]", "[黑傻沙煞撒]#2[逼比笔b]",
    "支#3那", "去#3搜", "共#3[产妻]", "毛#3东", "手#3银", "涩#3图", "肉#3棒", "邪#3教", "果#3聊", "裸#3体", "粉#3奶", "内#3射", "子#3宫", "排#3卵", "艾#3薇", "舔#3耳",
    ...
    "明#4[泽z]", "近#4[苹评]", "[小进]#4平", "[连再]#4任", "[看好]#4胸", "吃#4[比逼币笔]", "[日草艹操干曰吃]#4[姐妹奶姨吊弔]", 
    "留#5水", "性#5爱", "威#5雄",
    ...
    ### 字母+汉字（仅作简单处理）
    "si法", "你ma", "mei药", "媚yao", "吃shi", "lu#2发",  "看#3id", "加#4qq",
...
rules = {
    ...
    ### 中文非常规处理规则
    "([草艹操日曰干][ %s]*)([你您我他她它比笔逼]|时光|女儿)"%pm: lambda x: x.group(1)+sp+x.group(2),
    ...
    "(?<=[笨傻菜])[逼Bb]": "Ⲃ", "变态":"変态", "臭小鬼": "臭小鬼", "恶心心": "恶 心心",
    "%s死(?![\s2-79%s])"%(f1,cn): "si",
...
add_space_list=[
    ...
    "[你您他她这那个][们么样种些]?(?=[^\s\d个]{0,3}?[脸嘴鼻脑舌肚货猴胖矮丑烦笨傻蠢怪臭土歪睁垃混废脏Pp])",
    "[你您他这那]们?(?=.*?位.*?要求)",
    "[你您他她]们?(?=.*?东西)",
    "睁(?=[^\s睁]{0,3}?[开嘴脸鼻眼脑舌])",
    "[脸脑鼻口](?=.*?像)",
    ...
    "[日草艹操干曰死烧解透跳杀](?=.*?[你尼我他她它].*?[妈马吗码蚂玛犸嘛母m家])",
    "[你您他她男女人]们?[^\s人]?(?=[^\s\d]?(?:多.*?话|话.*?多|话%s))"%(f2),
    "你们?(?=[^\s\d吓笑]{0,6}死)",
    "死(?=\S?[心狗猪吧吗嘛啊呢哦么宅])",
    "[病笨傻蠢臭胖矮狗猪猴](?=\S{0,3}[样人吧吗嘛啊呢哦么了])",
    "[菜笨傻](?=.{0,2}[逼比笔币Bb])",
    "\d(?=.*?字母表)",
    "残(?=[疾障%s])"%(jp),
...
ex_words=[]
ex_rules={
    "臭": "臭", "恶": "悪", "滚": "gun", "妖": "yao", "傻": "sha", "狗": "gou", "脑":"nαo",
    "病": "Ⲃing", "蠢": "chun", "丑": "chou", "死": f"s{sp}i", "炸": "zha", "呕": "ou",
    "[你您他她它这那][们么样种些]?": add_space,
    "(?<!\s)[吗嘛啊吧呢哦]": lambda x: x.group(),
    "东西": "东 西",
}
add_space_string_ex="猴猪"+"嘴脸鼻眼舌肚"+"胖矮烦笨怪废睁混脏菜残"+"跟个样"
