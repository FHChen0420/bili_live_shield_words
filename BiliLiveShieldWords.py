...
words =  [ 
    ...
    "有神明", "一直播", "看名字", "报警了", "金小姐", "没试过", "小学生", "被墙了", "小裤裤",
    ...
    "的混合物", "波涛汹涌", "报复社会", "官方签约", "我还活着", "贤者模式", "恐怖游戏", "毫无意义", "自作自受", "侃侃而谈",
...
rules = {
    ...
    "咖喱 ?人": "咖喱/ 人",
    "全 ?家 ?炸": "全 家/炸",
    "母韵|太笨|整形|全家|变态|孤勇者": lambda x: x.group()[0]+" "+x.group()[1:],
    "%s(猴子|村人|爹妈|吃的吗|断[手脖])%s"%(f1,f3): lambda x: x.group()[0]+" "+x.group()[1:],
    "尼嚎": "你好",
    "快死": "快 "+sp+"死",
    "牲畜": "牲 1 畜",
    "人妖": "人 1 妖",
    ...
    # [AB]型
    "欧派|[肉尸]体|习[惯俗]|舰(?= ?[a-zA-Z])|[习習](?=.*?败)|[Bb]站?限定?|巴[啦拉]{2}小魔仙": lambda x: "["+x.group()+"]",
    "死(?= ?[吧吗嘛啊呢哦么了])": lambda x: "["+x.group()+"]",
    "(?<![\u4E00-\u9FA5])够了": lambda x: "["+x.group()+"]",
    "(?<![\u4E00-\u9FA5])熊(?![熊猫])": lambda x: "["+x.group()+"]",
    "(?<!臭)臭+(?![\u4E00-\u9FA5])": lambda x: "["+x.group()+"]",
    "(?<![0-9月])[0-9]+日(?![\u4E00-\u9FA5])": lambda x: "["+x.group()+"]",
    # [A_B]型
    "垃圾|人妻|整容|彩笔|狒狒|闭嘴|双亲|渣女|股间|矮子|小偷|愚蠢|脑子|臭小鬼|老太婆|滚[开滚]|你是个[Pp]|难[看听]": lambda x: "["+x.group()[0]+" "+x.group()[1:]+"]",
    # [A]_型
    ...
    "(?i)(?<![a-z])(s)(i)(?![a-z])": lambda x: x.group(1)+sp+x.group(2),
    "Z":"Ｚ","z":"ｚ",
    # <LD144 IGNORE BEGIN> #歌词机忽略的规则
    "原神|真白(?!花音)|日本人?": lambda x: "["+x.group()+"]",
    # <LD144 IGNORE END>
}
