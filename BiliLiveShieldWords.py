...
words =  [ 
    ...
    "[肉性]#1欲", "[条包孢窑梯秃矮棒牛]#1子", "[谢謝]#1罪",
    "[插吸]#1[你他她它]", "[红紅]#1[魔通]", "[鸡己几寄性基]#1[吧巴八8⑧]",
    "网#2恋", "巨#2乳", "自#2尽", "涩#2情", "翻#2墙", "蓝#2灯", "人#2权", "[逼b]#2[真黑毛美]", "[黑美傻沙煞撒]#2[逼比笔鼻b]",
    "支#3那", "去#3搜", "共#3[产妻]", "毛#3东", "手#3[银冲]", "涩#3图", "肉#3棒", "邪#3教", "果#3聊", "裸#3体", "[粉吃吸]#3奶", "内#3射", "子#3宫", "排#3卵", "艾#3薇", "舔#3耳", "光#3光", "法#3功", "大#3熊", "陪#3睡",
    "自#3[杀殺慰]", "鸡#3[巴八8⑧鸡]", "[涩色]#3网", "被#3[日草艹操曰]", "[习習吊弔]#3大", "[草艹操]#3[逼b]", "[逼b]#3内", "[下压]#3注", "[六6]#3中","[黄璜簧皇色卖h]#3片", "[做作坐座]#3[爱暖]", "[加+]#3[微薇v]", 
    "六#4四", "八#4九", "车#4震", "援#4交", "后#4入", "流#4世", "主#4席", "赤#4毒", "孤#4儿", "倒#4车", "约#4炮", "共#4共", "前#4妻", "看#4胸", "包#4力", "黄#4红", "[阴陰]#4道",
    "明#4[泽z]", "近#4[苹评]", "[小进]#4平", "[连再]#4任", "吃#4[比逼币笔]", "[日草艹操干曰]#4[姐妹奶姨吊弔妇]", 
    "留#5水", "性#5[爱交]", "威#5雄", "[黄璜簧皇]#5网", 
    "罗#6莉", "萝#6[利俐]", "丽#6丽", "宽#6衣", "枪#6[买卖]", "刀#6卖", "卖#6刀", "震#6好", "维#6尼", "敲#6锅", "胸#6腿", "彭#6s", "[彭澎p]#6帅", "[逼b]#6紧", "[习習]#6[近进]",
...
rules = {
    ...
    "A ?V": insert_filler,
    ### 中文特殊处理规则
    "[草艹操日曰干扣](?=[ %s]*[你您我他她它比笔逼妈父母]|时光|女儿|螃蟹)"%pm: add_filler,
    ...
    "[买卖].*?硬(?= ?币)": add_filler,
    "(小 ?学|[初高] ?中|年 ?级)(?=.*?[外语书政])": insert_filler, # 格式：甲#1乙#9丙(#1丁)
    ...
    "人妖": "人yaoᐨ", "咖喱 ?人": "咖喱renᐟ", "巨婴": "juᐠ婴", "弱智": "ruoᐠ智", "傻 ?子": "shaᘁ子", "奶 ?子": "naiᘁ子",
    "臭(?=[小老八弟])": "臭",
    "变(?=\S?态)": "変",
    "恶(?=\S?[心啊吧吗嘛么])": "悪",
    "母(?=的|亲[呢吗嘛])": "⺟",
...
list_1=[
    ...
    "(?<![小狗])狗(?![狗头熊仔])", "滚(?![滚动轴雪])",
    "[真太很好挺点个][^\s不真太很好挺点个]{0,3}(?:[胖矮肥傻笨蠢病丑废逊挫土]|老(?!师))",
]
