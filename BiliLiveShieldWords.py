...
rules = {
    ...
    # [AB]型
    "欧派|[肉尸]体|舰(?= ?[a-zA-Z])|[习習](?=.*?败)|[Bb]站?限定?": lambda x: "["+x.group()+"]",
    "[死赢动](?= ?[吧吗嘛啊呢哦么了])": lambda x: "["+x.group()+"]",
    "(?<![\u4E00-\u9FA5])够了": lambda x: "["+x.group()+"]",
    "(?<![0-9]月)[0-9]+日": lambda x: "["+x.group()+"]",
    # [A_B]型
    "垃圾|人妻|整容|彩笔|狒狒|闭嘴|双亲|渣女|股间|矮子|小偷|愚蠢|脑子|老太婆|你是个[Pp]|难[看听]": lambda x: "["+x.group()[0]+" "+x.group()[1:]+"]",
    # [A]_型
    ...
    "原神|阿姨|身体|透过|真白(?!花音)|日本人?": lambda x: "["+x.group()+"]",
    # <LD144 IGNORE END>
}
