...
words =  [ 
    ...
    "bajiu", "luoli", "sager", "shina", "hentai", "signal", "youtube", "revolution", "ilibilib", "pilipili", "dilidili", 
    "404", "535", "604", "881", "918", "1921", "1926", "1953", "1979", "1989", "g7", "g20", "j8",
    "不想活", "自由门", "咖啡因", "死灵魂", "白衬衫", "生理期", "空气炮", "黑历史", "就去泡", "一本道", "被传染", "网易云",
    "纪念日", "为自由", "莉莉安", "李医生", "右大人", "绞肉机", "不唱歌", "女菩萨", "发不出", "老鼠台", "缘之空", "一起死",
    "四一二", "九一八", "九四六", "九五四", "九七一", "九七五", "九九六", "一九二六", "一九五三",
    ...
    "手#5指#5插", "文#6古#6花", "看#6地#6方", "不#6钱#6啊", "那#1个#4奶#1奶", "清#6透#6世#6界", "w#4e#4i#4b#4o", "一#6个#6人#6寂#6寞",
    "[贫小平]#1乳", "色#1[逼b]", "[傻沙煞撒]#1[逼比笔]", "[插吸]#1[你他她它]", "称#1[王皇帝]", "[.。·]#1[cf]", "点#1[人1cfl]", 
    "巨#2乳", "[草操]#3b", "被#3[日草操]", "少女#3[下自]", "[做作坐座]#3爱", "去#3搜", "下#3注", "百#3d", "光#3光",
...
rules = {
    ...
    "(.? ?7 ?)(3 ?.?)": lambda x: (x.group(1)+"\u0592"+x.group(2)) if re.match("^\\W? ?7 ?$",x.group(1),re.A) and re.match("^3 ?\\W?$",x.group(2),re.A) else x.group(),
    "(.? ?6 ?)(4 ?.?)": lambda x: (x.group(1)+"\u0592"+x.group(2)) if re.match("^\\W? ?6 ?$",x.group(1),re.A) and re.match("^4 ?\\W?$",x.group(2),re.A) else x.group(),
    "(.? ?8 ?)(9)": lambda x: (x.group(1)+"\u0592"*2+x.group(2)) if re.match("^\\W? ?8 ?$",x.group(1),re.A) else x.group(),
