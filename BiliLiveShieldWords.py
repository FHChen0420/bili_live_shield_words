...
words =  [ 
    ...
    "天安", "安门", "审查", "屏蔽", "番号", "之那", "赤字",
    "生事", "闹事", "闹剧", "游行", "颠覆", "起义", "抗争", "革命",
    "杀死", "囚禁", "施虐", "虐待", "捆绑", "割腕", "匕首", "疫情", "新冠", 
    "大麻", "包粉", "白粉",
    "色戒", "湿了", "射了", "硬了", "中出", "高潮", "调教", "被透", "走光", "诱惑",
    "喘气", "喘息", "呻吟", "处女", "处男", "绅士", "抖m" , "h漫" , "性癖", "无码",
    "胖次", "欧派", "罩杯", "大胸", "嘿咻", "下女", "内内", "吹气", "掏耳", "助眠",
    "涩气", "色皮", "色批", "涩p" , "涩批", "重口", "射爆", "勃起", "出轨",
    "肥猪", "下贱", "死吧", "你妈", "死妈", "去死", "月半", "己吧", "送妈", "死法",
    "si法", 
    "美国", "米国", "台湾", "香港", "澳门", "成都", "日吹", "精美", "精日", "弯弯", 
    "油管", "推特", "微博", "新浪", "抖音", "优酷", "鸣人", "动森", "梯子",
    "皇帝", "皇宫", "庙堂", "磕头", "传教", "宗教", "还愿", "安乐", "包养", "一哥",
    "赌博", "扑克", "彩票", "发票", "博彩", "菠菜", "硫酸", "甲烷", "煤气",
    "四六", "五四", "七一", "七五", "九八",
    "爸爸", "即为", "试看", "豪迈", "耳语", "迷途", "触摸", "初心", "慎重", "三尺",
    "许愿", "代打", "躺平", "要素", "精湛", "上街", "读错", "性吧",
    "与正", "蒂亚", "稻上", "飞草", "熊学", "伐龙", "家明", "马云",
    "ロリ", "はま", "ハマ", "しな", "シナ", "くま", "死ね", "映画", 

    "hw", "gc", "zf", "cnm", "kui", "lsp", "nmd", "tmd", "usl", "xxd", "boki", "drug", "knee", "kuma", "loli", "sina", 
    "bajiu", "luoli", "sager", "shina", "hentai", "reddit", "signal", "tiktok", "youtube",
    "ilibilib", "pilipili", "dilidili", "revolution",
    "404", "535", "586", "604", "809", "881", "918", "1921", "1926", "1953", "1979", "1989", "g7", "g20", "j8",
    ...
    "自由之门", "继续前进", "并肩同行", "焕然一新", "奥斯曼人", "二氧化碳", "阿里巴巴", "恐怖分子", "恐怖份子", "田所浩二", "b站员工" ,
    ...
    # "书记", "想死", "许愿", "啪啪", "啪啪啪", "点点点", "64", "73", "89", #其余部分见rules 

    ### 以下词汇屏蔽已失效
    # "神社", "垃圾", "人妻", "改变", "签约", "失望", "控制", "节奏", "赤裸", "天城",
    # "黑手", "集会", "光荣", "虾膜", "成人", "中央", "万岁", "萝莉",
    # "71", "av", "sb", "小学生", "不习惯", "毕业歌", "风平浪静", "老不死的",

    ### 字符间隔相关
    "回#1来#3谢", "姐#1姐#5[逼b]", "妹#1妹#5[逼b]", "看#3i#1d", "[逼b]#3看#1吗", "搜#3这#1个",
    "道#3上#3飞", "名#3字#3看", "我#3是#3处", "下#3面#3好", "白#3一#3下", "少#3女#3[下自]", "直#3播#3[日草操]", 
    "射#4身#1上", "v#4b#3o", "天#4安#4门", "萝#4莉#4控", "死#4[你他她]#5[吗嘛妈马]", "手#5指#5插", 
    "[日草操干]#6[你我他她它]#5[妈吗马嘛]", "文#6古#6花", "看#6地#6方", "不#6钱#6啊",
    "那#1个#4奶#1奶", "羊#1羊#4结#1婚", "学#1生#4学#1生", "r#1i#6[你我他她它]#5[妈吗马嘛]", "清#6透#6世#6界", "w#4e#4i#4b#4o", "一#6个#6人#6寂#6寞",
    "[贫小平]#1乳", "色#1[逼b]", "[逼b]#1里", "[傻沙煞撒]#1[逼比笔]", "[插吸]#1[你他她它]", "称#1[王皇帝]", "[.。·]#1[cf]", "点#1[人1cfl]", 
    "巨#2乳", "自#2尽", "[草操]#3b", "被#3[日草操]", "[做作坐座]#3爱", "支#3那", "去#3搜", "[下压]#3注", "百#3d", "h#3片", "自#3杀", "光#3光",
    "六#4四", "八#4九", "[日草操干]#4[姐妹奶]", "[看好]#4胸", "车#4震", "小#4平", "再#4任", "明#4泽", "援#4交", "后#4入", "天#4门", "流#4世", 
    "留#5水", "性#5爱", "威#5雄", "萝#6[利俐]", "罗#6莉",
...
rules = {
    ...
    "(?a)(?<!\\w)(6 ?)(4)(?!\\w)": lambda x: x.group(1)+"\u0592"+x.group(2), # 下列三条规则没法处理前方有空格的情况，想要处理好的话得分组再匹配一次
    "(?a)(?<!\\w)(7 ?)(3)(?!\\w)": lambda x: x.group(1)+"\u0592"+x.group(2),
    "(?a)(?<!\\w)(8 ?)(9)": lambda x: x.group(1)+"\u0592"*2+x.group(2),
    ...
    "(?i)([%sail].*?)([就上去还点被了].*?)([来射车有点].*)"%(hz_du): lambda x:
        (x.group(1)+fill(x.group(2),5+r_pos(x.group(2),"就上去还点被了"))+x.group(3))
        if measure(x.group(1),7) and measure(x.group(2),5+r_pos(x.group(2),"就上去还点被了"))
